...
class CopilotService:
    ...
    async def chat(
        ...
        # Autocorrect phonetic/transcription errors in the query if it was generated by speech
        corrected_query = query
        try:
            correction_prompt = f"""You are a speech-recognition correction assistant for Informant (an AI browser co-pilot). 
Analyze the user's spoken query and fix any phonetic, spelling, or transcription errors, mapping them back to correct context like "VideoDB", "DIV Fund", "hackathon", "co-pilot", "Informant", or the active page context.

PAGE CONTEXT:
- Title: {page_title}
- URL: {page_url}

ORIGINAL VOICE QUERY: "{query}"

Return ONLY the corrected query text, or the original query if no correction is needed. Do not wrap in quotes or add explanations."""
            correction_res = await ai_service.generate_content_async(correction_prompt)
            if correction_res and correction_res.strip():
                candidate = correction_res.strip().replace('"', '')
                if candidate != query:
                    logger.info("Speech recognition query corrected", original=query, corrected=candidate)
                    corrected_query = candidate
        except Exception as e:
            logger.warning("Failed to correct speech query", error=str(e))
